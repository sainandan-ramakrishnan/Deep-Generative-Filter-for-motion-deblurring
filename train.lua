...
opt = {
   ...
   batchSize = 1,          -- # images in batch  increase this as much as your GPU memory allows. We did not observe significant changes in perofrmance. 
   loadSize = 256,         -- scale images to this size
   fineSize = 256,         --  then crop to this size
   ngf = 64,               -- #  of gen filters in first conv layer
   ndf = 48,               -- #  of discrim filters in first conv layer # default = 48
   display = 1,
   input_nc = 3,           -- #  of input image channels
   output_nc = 3,          -- #  of output image channels
   niter = 100000000,            -- #  of iter at starting learning rate
   ...
   name = 'deshake',              -- name of the experiment, should generally be passed on the command line
   which_direction = 'AtoB',    -- AtoB or BtoA
   phase = 'training',             -- train, val, test, etc
   preprocess = 'regular',      -- for special purpose preprocessing, e.g., for colorization, change this (selects preprocessing functions in util.lua)
   nThreads = 1,                -- # threads for loading data
   save_epoch_freq = 0,        -- save a model every save_epoch_freq epochs (does not overwrite previously saved models)
   save_latest_freq = 2500,     -- save the latest model every latest_freq sgd iterations (overwrites the previous latest model)
   print_freq = 50,             -- print the debug information every print_freq iterations
   display_freq = 30,          -- display the current results every display_freq iterations
   ...
   lambda = 145,               -- weight on Perceptual loss term in objective
   lambda1 = 170,               -- weight on L1 term in objective        # default = 150 
   bottleneck = 1,              -- 1 for bottleneck layer required, 0 to turn off bottleneck
   growthRate = 64,             -- GR, number of output channels at the end of each 3*3 convolutional module inside a dense block
   dropRate = 0.5,
   depth = 64,                  -- effective depth of our dense net with bottleneck = 6*N + 4 ( Default N = 11 ) 
   optMemory = 4,               -- for memory optimization ( 4 = highly optimized, 1 = inefficient/standard ) 
...
function defineG(input_nc, output_nc, ngf, nz)
    if opt.which_model_netG == "our_net" then netG = defineGour_net(input_nc, output_nc, ngf,opt)
    ...
    for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
        ...
        if counter % opt.display_freq == 0 and opt.display then
            ...
            if opt.preprocess == 'colorization' then 
                local real_A_s = util.scaleBatch(real_A:float(),512,512)
                local fake_B_s = util.scaleBatch(fake_B:float(),512,512)
                local real_B_s = util.scaleBatch(real_B:float(),512,512)
                ...
                disp.image(util.deprocess_batch(util.scaleBatch(real_A:float(),512,512)), {win=opt.display_id, title=opt.name .. ' input'})
                disp.image(util.deprocess_batch(util.scaleBatch(fake_B:float(),512,512)), {win=opt.display_id+1, title=opt.name .. ' output'})
                disp.image(util.deprocess_batch(util.scaleBatch(real_B:float(),512,512)), {win=opt.display_id+2, title=opt.name .. ' target'})
            end
        end
        ...
        local loss = {errG=errG and errG or -1, errD=errD and errD or -1, errLp=errLp and errLp or -1}
