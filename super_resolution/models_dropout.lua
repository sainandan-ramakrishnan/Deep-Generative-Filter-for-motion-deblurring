...
local function backbone(res_blocks)
      local inner_skip = nn.Sequential()
      local highway = nn.Identity()
      inner_skip:add(layer(opt.residual_blocks))
      inner_skip:add(nn.SpatialConvolution(64,64,3,3,1,1,1,1))
      inner_skip:add(nn.SpatialBatchNormalization(64))
...
function M.build_model(opt)
  ...
  model:add(backbone(opt.residual_blocks))
