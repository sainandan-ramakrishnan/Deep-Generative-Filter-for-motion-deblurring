...
function DenseConnectLayerStandard(nChannels, opt)
   ...
   if opt.bottleneck then
      net:add(nn.SpatialConvolution(nChannels, 4 * opt.growthRate, 1, 1, 1, 1, 0, 0))
      nChannels = 4 * opt.growthRate
      --if opt.dropRate > 0 then net:add(nn.Dropout(opt.dropRate)) end
      net:add(nn.SpatialBatchNormalization(nChannels))
...
function LastLayer(nChannels, opt)
   local net = nn.Sequential()

   --net:add(ShareGradInput(nn.SpatialBatchNormalization(nChannels), 'first'))
   --if opt.dropRate > 0 then self.net:add(nn.Dropout(opt.dropRate)) end
   net:add(nn.LeakyReLU(0.2,true))   
   if opt.bottleneck then
      net:add(nn.SpatialConvolution(nChannels, 4 * opt.growthRate, 1, 1, 1, 1, 0, 0))
      nChannels = 4 * opt.growthRate
   ...
   --net:add(nn.SpatialConvolution(nChannels, 3, 3, 3, 1, 1, 1, 1))
...
function DenseConnectLayerCustom:__init(nChannels,opt,k)
   parent.__init(self)
   self.train = true
   self.opt = opt

   self.net1 = nn.Sequential()
   --self.net1:add(ShareGradInput(nn.SpatialBatchNormalization(nChannels), 'first'))
   self.net1:add(nn.LeakyReLU(0.2,true))  

   self.net2 = nn.Sequential()
   if opt.bottleneck then
      self.net2:add(nn.SpatialConvolution(nChannels, 4*opt.growthRate, 1, 1, 1, 1, 0, 0))
      nChannels = 4 * opt.growthRate
      self.net2:add(nn.SpatialBatchNormalization(nChannels))
      self.net2:add(nn.LeakyReLU(0.2,true))
   end
   if k%2 == 0 then
      self.net2:add(nn.SpatialDilatedConvolution(nChannels, opt.growthRate, 3, 3, 1, 1, 2, 2, 2, 2))
   else
      self.net2:add(nn.SpatialDilatedConvolution(nChannels, opt.growthRate, 3, 3, 1, 1, 2, 2, 1, 1))
   end

   self.net2:add(nn.SpatialBatchNormalization(opt.growthRate))
   --if opt.dropRate > 0 then self.net2:add(nn.Dropout(opt.dropRate)) end
